...
| Section | Step File |
|---------|-----------|
| Section 4: Hello World | `steps/step04_hello_agent.cs` |
| Section 6: Hardcoded Jokes | `steps/step06_joke_agent.cs` |
| Section 7: Live API Jokes | `steps/step07_joke_agent.cs` |
| Section 8: Weather DataMap | `steps/step08_weather_joke_agent.cs` |
| Section 9: Polish | `steps/step09_weather_joke_agent.cs` |
| Section 10: Skills | `steps/step10_weather_joke_agent.cs` |
| Section 11: Complete Agent | `steps/step11_complete_agent.cs` |
...
This builds the SignalWire .NET SDK targeting .NET 8.0 and gets your project ready to go. A `.env` file will be created (if one doesn't exist) with your environment variables.
...
> See [steps/step04_hello_agent.cs](steps/step04_hello_agent.cs)

Let's break down what's happening:

- **Loading `.env`** -- the code reads your `.env` file line by line, parsing key-value pairs and setting them as environment variables via `Environment.SetEnvironmentVariable()`. This is the C# equivalent of Python's `load_dotenv()`.
- **ngrok auto-detection** -- the code queries ngrok's local API at `http://127.0.0.1:4040/api/tunnels` using `HttpClient`. If ngrok is running, it automatically sets `SWML_PROXY_URL_BASE` -- the environment variable the SDK uses to generate correct webhook URLs. If ngrok isn't running yet (it isn't -- we'll set it up in Section 5), it prints a helpful message and moves on. No manual URL configuration needed.
- **`AgentBase`** is the foundation class for every agent, constructed with `AgentOptions`
...
```bash
swaig-test steps/step04_hello_agent.cs --dump-swml
```
...
```bash
dotnet run steps/step04_hello_agent.cs
```
...
INFO: Now listening on: http://0.0.0.0:3000
```
...
> **Checkpoint:** You see SWML JSON output from both `swaig-test` and curl. The JSON contains your prompt text and voice settings. If not, double-check that your `.env` file exists in the working directory and that .NET 8.0+ is installed.
...
The ngrok detection code we wrote earlier just found your tunnel automatically. No need to manually configure `SWML_PROXY_URL_BASE` -- the agent discovers it on every startup.
...
> See [steps/step06_joke_agent.cs](steps/step06_joke_agent.cs)
...
- **`parameters`** defines what the AI should extract from the conversation. Our joke function doesn't need any input, so it's an empty `Dictionary<string, object>`.
- The **handler** is a lambda `(args, rawData) => { ... }` that receives the extracted arguments and returns a `FunctionResult`.
- A **`jokes` array** holds three hardcoded dad jokes, and `Random.Shared.Next()` picks one at random.
...
```bash
swaig-test steps/step06_joke_agent.cs --list-tools
```

You should see `tell_joke` listed. Now test executing it:

```bash
swaig-test steps/step06_joke_agent.cs --exec tell_joke
```
...
```bash
dotnet run steps/step06_joke_agent.cs
```
...
> See [steps/step07_joke_agent.cs](steps/step07_joke_agent.cs)

What changed from the hardcoded version:

- Removed the `jokes` array and `Random.Shared` usage
- Added an `HttpClient` instance (`jokeClient`) for making API calls
- The handler now calls the API Ninjas endpoint using `HttpRequestMessage` and adds the `X-Api-Key` header
- We read the API key from the environment with `Environment.GetEnvironmentVariable("API_NINJAS_KEY")`
- The response JSON is deserialized with `JsonSerializer.Deserialize<JsonElement>()` and the joke is extracted from the first array element
- There's error handling -- if the API is down or the key is wrong, the agent says something graceful instead of crashing

### Test It

```bash
swaig-test steps/step07_joke_agent.cs --exec tell_joke
```
...
```bash
dotnet run steps/step07_joke_agent.cs
```
...
> See [steps/step08_weather_joke_agent.cs](steps/step08_weather_joke_agent.cs)
...
The API key is baked into the URL at startup time (via the C# string interpolation `$"...{weatherKey}..."`). The city gets substituted at call time (via `${enc:args.city}`). Note the `${{enc:args.city}}` syntax in the source -- the double braces `{{` and `}}` escape the curly braces so C#'s string interpolation doesn't try to evaluate them.

Finally, **`agent.RegisterSwaigFunction(weather.ToSwaigFunction())`** adds the DataMap to the agent.

### Test It

```bash
swaig-test steps/step08_weather_joke_agent.cs --list-tools
```

You should see both `tell_joke` and `get_weather`. Now look at how the DataMap appears in the SWML:

```bash
swaig-test steps/step08_weather_joke_agent.cs --dump-swml
```
...
```bash
swaig-test steps/step08_weather_joke_agent.cs --exec tell_joke
```
...
```bash
dotnet run steps/step08_weather_joke_agent.cs
```
...
Your agent works, but it sounds a bit robotic. Let's give it a personality and tune the conversation flow. Same capabilities, better experience.

> See [steps/step09_weather_joke_agent.cs](steps/step09_weather_joke_agent.cs)

What we improved:

- **`SetParams()`** -- `end_of_speech_timeout` of 600ms means the agent waits a natural beat before responding (not jumping in too fast). `attention_timeout` of 15 seconds prompts the caller if they go quiet. These are passed as a `Dictionary<string, object>`.
- **`AddHints()`** -- helps the speech recognizer with words it might mishear. "Buddy" could sound like "body" without a hint. Passed as a `List<string>`.
- **Richer prompts** -- the "Personality" section gives the AI a character to play. The "Voice Style" section has specific rules for phone conversation, passed as a `List<string>` of bullet points. The "Capabilities" section tells the AI what tools it has.
- **Improved post-prompt** -- now asks the AI to note what topics the caller asked about.

### Test and Call

```bash
swaig-test steps/step09_weather_joke_agent.cs --dump-swml
```

Look at the SWML -- you'll see the `hints` array, the `params` section with your timeouts, and the richer prompt. Restart and call:

```bash
dotnet run steps/step09_weather_joke_agent.cs
```
...
> See [steps/step10_weather_joke_agent.cs](steps/step10_weather_joke_agent.cs)
...
```bash
swaig-test steps/step10_weather_joke_agent.cs --list-tools
```

You should see your original two functions plus the skill functions: `get_current_time`, `get_current_date`, and `calculate`.

```bash
swaig-test steps/step10_weather_joke_agent.cs --exec get_current_time
swaig-test steps/step10_weather_joke_agent.cs --exec calculate --expression "15/100 * 47.50"
```
...
```bash
dotnet run steps/step10_weather_joke_agent.cs
```
...
Let's bring everything together into one clean, final version. This is the definitive `complete_agent.cs` -- combining all four capabilities with polished prompts and tuned parameters.

> See [steps/step11_complete_agent.cs](steps/step11_complete_agent.cs)
...
- **.env loading** -- reads credentials from your `.env` file
- **ngrok auto-detection** -- discovers the tunnel URL and prints the full SWML URL with auth credentials
- **Voice and speech** -- `AddLanguage()`, `AddHints()` for speech recognition
- **AI parameters** -- `SetParams()` for conversation tuning
- **Prompts** -- personality, voice style, capabilities, and greeting sections
...
This pattern -- top-level statements with clearly separated sections -- is the standard way to organize agents in C# using the .NET SDK's minimal API style.
...
swaig-test steps/step11_complete_agent.cs --dump-swml

# List all tools
swaig-test steps/step11_complete_agent.cs --list-tools

# Test individual functions
swaig-test steps/step11_complete_agent.cs --exec tell_joke
swaig-test steps/step11_complete_agent.cs --exec get_current_time
swaig-test steps/step11_complete_agent.cs --exec calculate --expression "20 * 1.15"
```

### Go Live

```bash
dotnet run steps/step11_complete_agent.cs
```
...
```bash
swaig-test your_agent.cs --dump-swml          # View full SWML configuration
swaig-test your_agent.cs --list-tools         # List registered functions
swaig-test your_agent.cs --exec func_name     # Execute a function
swaig-test your_agent.cs --exec calc --expression "2+2"  # With arguments
```
...
agent.DefineTool(
    ...
    handler: (args, rawData) => new FunctionResult("...")
);
...
agent.AddSkill("skill_name", new Dictionary<string, object> { ["key"] = "value" });
```

### Imports

```csharp
using SignalWire.Agent;    // AgentBase, AgentOptions
using SignalWire.SWAIG;    // FunctionResult, DefineTool
using SignalWire.DataMap;  // DataMap
using System.Text.Json;    // JsonSerializer, JsonDocument, JsonElement
```
...
| Problem | Solution |
|---------|----------|
| Agent won't start | Check .NET 8.0+ is installed (`dotnet --version`) |
| Build errors | Run `dotnet build` and check for missing references |
...
| `.env` not loading | Ensure `.env` is in the working directory or `AppContext.BaseDirectory` |
