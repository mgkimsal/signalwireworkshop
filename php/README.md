...
| Section | Step File |
|---------|-----------|
| Section 4: Hello World | `steps/step04_hello_agent.php` |
| Section 6: Hardcoded Jokes | `steps/step06_joke_agent.php` |
| Section 7: Live API Jokes | `steps/step07_joke_agent.php` |
| Section 8: Weather DataMap | `steps/step08_weather_joke_agent.php` |
| Section 9: Polish | `steps/step09_weather_joke_agent.php` |
| Section 10: Skills | `steps/step10_weather_joke_agent.php` |
| Section 11: Complete Agent | `steps/step11_complete_agent.php` |
...
> [!NOTE]
> **Docker users:** The setup script has already run inside the container. Just run `cd php` and skip to Section 4.
...
This clones the SignalWire PHP SDK and installs its dependencies via Composer. It also creates a `.env` file if one doesn't exist.
...
> See [steps/step04_hello_agent.php](steps/step04_hello_agent.php)

Let's break down what's happening:

- The `.env` loader reads your `.env` file line by line, calling `putenv()` so environment variables are available throughout the script
...
- `AgentBase` is the foundation class for every agent. We import it with `use SignalWire\Agent\AgentBase`
...
- `$agent->run()` starts the built-in web server on port 3000
...
```bash
swaig-test steps/step04_hello_agent.php --dump-swml
```
...
```bash
php steps/step04_hello_agent.php
```
...
```bash
curl -s -u workshop:pickASecurePassword123 http://localhost:3000/ | php -r 'echo json_encode(json_decode(file_get_contents("php://stdin")), JSON_PRETTY_PRINT);'
```
...
> **Checkpoint:** You see SWML JSON output from both `swaig-test` and curl. The JSON contains your prompt text and voice settings. If not, double-check that your `.env` file exists in the `php/` directory and that the autoload path is correct (`require __DIR__ . '/../../sdks/signalwire-php/vendor/autoload.php'`).
...
```bash
curl -s -u workshop:pickASecurePassword123 https://your-domain.ngrok-free.app/ | php -r 'echo json_encode(json_decode(file_get_contents("php://stdin")), JSON_PRETTY_PRINT);'
```
...
> See [steps/step06_joke_agent.php](steps/step06_joke_agent.php)

Let's look at the new pieces:

- `FunctionResult` is how you return data from a SWAIG function. The AI takes this text and weaves it into its response. We import it with `use SignalWire\SWAIG\FunctionResult`.
- `defineTool()` registers the function. The `description` is critical -- it tells the AI *when* to call this function.
- `parameters` defines what the AI should extract from the conversation. Our joke function doesn't need any input, so it's `['type' => 'object', 'properties' => []]`.
- The `handler` is a closure that receives `$args` (parsed parameters) and `$rawData` (the full request payload), and returns a `FunctionResult`.
- Inside the handler, we pick a random joke from a PHP array using `$jokes[array_rand($jokes)]`.
...
```bash
swaig-test steps/step06_joke_agent.php --list-tools
```

You should see `tell_joke` listed. Now test executing it:

```bash
swaig-test steps/step06_joke_agent.php --exec tell_joke
```
...
```bash
php steps/step06_joke_agent.php
```
...
```bash
curl -s -H "X-Api-Key: YOUR_API_NINJAS_KEY" https://api.api-ninjas.com/v1/dadjokes | php -r 'echo json_encode(json_decode(file_get_contents("php://stdin")), JSON_PRETTY_PRINT);'
```

### Update the Joke Agent

> See [steps/step07_joke_agent.php](steps/step07_joke_agent.php)

What changed from the hardcoded version:

- Removed the `$jokes` array and `array_rand()` call
- The handler now calls the API Ninjas endpoint using `file_get_contents()` with a `stream_context_create()` for the HTTP headers
- We read the API key from the environment with `getenv('API_NINJAS_KEY')`
- There's error handling -- if the API key is missing, the API is down, or the response is empty, the agent says something graceful instead of crashing

Here's the key part of the handler:
...
PHP's `stream_context_create()` lets us set HTTP headers and timeouts without needing cURL or an external HTTP library. The `@` suppresses warnings if the request fails, and we check the result before using it.

### Test It

```bash
swaig-test steps/step07_joke_agent.php --exec tell_joke
```
...
```bash
php steps/step07_joke_agent.php
```
...
For the joke function, you wrote PHP code that runs on your server. That works great, but there's another way: **DataMap**.
...
> See [steps/step08_weather_joke_agent.php](steps/step08_weather_joke_agent.php)
...
- `new DataMap('get_weather')` -- creates a new DataMap function with that name. We import it with `use SignalWire\DataMap\DataMap`.
- `->description(...)` -- tells the AI when to use it (same idea as `defineTool`)
- `->parameter('city', 'string', ...)` -- the AI will extract the city from the caller's request. The `required: true` named argument uses PHP 8.0+ syntax.
- `->webhook('GET', $url)` -- the HTTP request SignalWire will make. Notice `\${enc:args.city}` -- that's the city parameter, URL-encoded, inserted right into the URL. We escape the `$` with a backslash so PHP doesn't try to interpolate it.
...
- `->toSwaigFunction()` converts the DataMap into a format the agent can register

The API key is baked into the URL at startup time (via PHP string interpolation `{$weatherKey}`). The city gets substituted at call time (via `${enc:args.city}`).

### Test It

```bash
swaig-test steps/step08_weather_joke_agent.php --list-tools
```

You should see both `tell_joke` and `get_weather`. Now look at how the DataMap appears in the SWML:

```bash
swaig-test steps/step08_weather_joke_agent.php --dump-swml
```
...
```bash
swaig-test steps/step08_weather_joke_agent.php --exec tell_joke
```
...
```bash
php steps/step08_weather_joke_agent.php
```
...
> See [steps/step09_weather_joke_agent.php](steps/step09_weather_joke_agent.php)

What we improved:

- **`setParams()`** -- `end_of_speech_timeout` of 600ms means the agent waits a natural beat before responding (not jumping in too fast). `attention_timeout` of 15000ms (15 seconds) prompts the caller if they go quiet.
- **`addHints()`** -- helps the speech recognizer with words it might mishear. "Buddy" could sound like "body" without a hint.
- **Richer prompts** -- the "Personality" section gives the AI a character to play. The "Voice Style" section has specific rules for phone conversation as a bulleted list using the `bullets:` named argument. The "Capabilities" section tells the AI what tools it has.

Here's how the params and hints look in PHP:
...
And the structured prompt with bullets:

```php
$agent->promptAddSection('Voice Style', 'Since this is a phone conversation, follow these rules:', bullets: [
...
> See [steps/step10_weather_joke_agent.php](steps/step10_weather_joke_agent.php)
...
|-----------|----------|---------------|------------------------|
| Dad Jokes | `defineTool` | ~20 lines | Yes |
...
```bash
swaig-test steps/step10_weather_joke_agent.php --list-tools
```

You should see your original two functions plus the skill functions: `get_current_time`, `get_current_date`, and `calculate`.

```bash
swaig-test steps/step10_weather_joke_agent.php --exec get_current_time
swaig-test steps/step10_weather_joke_agent.php --exec calculate --expression "15/100 * 47.50"
```
...
```bash
php steps/step10_weather_joke_agent.php
```
...
Let's bring everything together into one clean, final version. This is the definitive `complete_agent.php` -- combining all four capabilities with polished prompts and tuned parameters.

> See [steps/step11_complete_agent.php](steps/step11_complete_agent.php)

### What's Different From the Iterative Version?

Structurally, very little. This is the same agent you've been building, just organized into clean, well-commented sections:

- **Voice and speech** -- language, hints
- **AI parameters** -- conversation flow tuning
- **Prompts** -- personality, voice style, capabilities, greeting
- **Dad jokes** -- custom `defineTool` calling API Ninjas
- **Weather** -- serverless DataMap via WeatherAPI
- **Skills** -- built-in datetime and math
- **Post-prompt** -- call summaries saved to `calls/`

The complete agent also adds a "Greeting" prompt section that tells the AI how to open the conversation:
...
This pattern -- grouping related configuration into clearly labeled sections -- is the standard way to organize larger agents in the SDK.
...
swaig-test steps/step11_complete_agent.php --dump-swml

# List all tools
swaig-test steps/step11_complete_agent.php --list-tools

# Test individual functions
swaig-test steps/step11_complete_agent.php --exec tell_joke
swaig-test steps/step11_complete_agent.php --exec get_current_time
swaig-test steps/step11_complete_agent.php --exec calculate --expression "20 * 1.15"
```

### Go Live

```bash
php steps/step11_complete_agent.php
```
...
```bash
swaig-test your_agent.php --dump-swml          # View full SWML configuration
swaig-test your_agent.php --list-tools         # List registered functions
swaig-test your_agent.php --exec func_name     # Execute a function
swaig-test your_agent.php --exec calc --expression "2+2"  # With arguments
```
...
$dm = (new DataMap('name'))->description('...')->parameter(...)->webhook(...)->output(...);
$agent->registerSwaigFunction($dm->toSwaigFunction());

// 3. Skill (pre-built, one line)
$agent->addSkill('skill_name', ['config' => 'value']);
```
...
| Problem | Solution |
|---------|----------|
| Agent won't start | Check PHP 8.1+ is installed and Composer dependencies are installed |
| `Class not found` errors | Verify the `require` path to `vendor/autoload.php` is correct |
